	...
	*/
	...
	GameCtrl.Arena.prototype = {
		...
		create: function () {
			...
			playerHUD=this.add.text(100, 100, 'Enemies left: '+actorList.length-1, style);
			...
			if (victim.hp <= 0) {
				...
				if(victim!==player) {
					if (actorList.length === 1) {
						// victory message
						var victory = game.add.text(game.world.centerX, game.world.centerY, 'Victory! Not Today Cars!!!\nCtrl+r to restart', { fill : '#2e2', align: 'center' } );
	...
	var HUD={
		...
		msg:function(text, sprite, speed, color){
			...
			this.game.add.tween(text).to( { alpha: 1 }, Math.floor(speed*0.75), Phaser.Easing.Linear.None, true).to( { alpha: 0 }, Math.floor(speed*0.25), Phaser.Easing.Linear.None, true);
